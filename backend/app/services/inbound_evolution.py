...
A Evolution pode enviar `messages.upsert` com:
- `data` como um único objeto com `key` + `message`, ou
- `data.messages` como lista de mensagens, ou
- `data` como lista em algumas versões.
...
def _iter_message_blocks(payload: dict[str, Any]) -> list[dict[str, Any]]:
    """Normaliza payload Evolution em lista de blocos { key, message, ... }."""
    ...
    if isinstance(data, dict):
        ...
        return [data]
    # Alguns proxies enviam o evento no root
...
def extract_inbound_text_and_phone(payload: dict[str, Any]) -> tuple[str, str] | None:
    """
    Extrai (texto, telefone_só_dígitos) da primeira mensagem **recebida** (não enviada pela instância).
    Retorna None se não houver mensagem de texto/caption utilizável ou telefone.
    """
    # Atalho para testes manuais / integrações simples
    ...
    if not blocks:
        logger.debug("inbound_evolution: nenhum bloco de mensagem (event=%s keys=%s)", event, list(payload.keys()))
        return None
    ...
    for block in blocks:
        ...
        if not text:
            # Alguns payloads trazem texto em outro nível
    ...
    logger.debug(
        "inbound_evolution: blocos sem texto+telefone inbound (event=%s n_blocks=%s)",
        event,
        len(blocks),
    )
    return None
...
def phones_match_for_lead(inbound_digits: str, stored_phone: str) -> bool:
    """
    Compara telefone do WhatsApp com o salvo no lead (máscara, com/sem 55, etc.).
    """
    a = inbound_digits
    ...
    # Últimos 11 dígitos (DDD + celular BR)
    if len(a2) >= 11 and len(b2) >= 11 and a2[-11:] == b2[-11:]:
        return True
    # Últimos 9 (só número móvel sem DDD)
