...
def _default_scoring_rules() -> dict[str, dict[str, int]]:
    return {
        "A": {
            "cartao": 20,
...
def _default_classification_rules() -> dict[str, int]:
    return {
...
def _ensure_config(db: Session, tenant_id: int, campaign_id: int) -> CampaignQualificationConfig:
    cfg = (
    ...
    cfg = CampaignQualificationConfig(
        ...
        enabled=True,
        questions_json=_default_questions(),
        scoring_rules_json=_default_scoring_rules(),
        classification_rules_json=_default_classification_rules(),
...
def _ordered_steps(cfg: CampaignQualificationConfig) -> list[str]:
...
def _classify(score: int, rules: dict[str, Any]) -> str:
...
def _score_answer(step_key: str, answer: str, scoring_rules: dict[str, Any]) -> tuple[int, str]:
...
def get_campaign_qualification_config(
    ...
    cfg = _ensure_config(db, tenant_id, campaign_id)
...
def put_campaign_qualification_config(
    ...
    cfg.questions_json = body.questions_json or _default_questions()
    cfg.scoring_rules_json = body.scoring_rules_json or _default_scoring_rules()
    cfg.classification_rules_json = body.classification_rules_json or _default_classification_rules()
...
async def post_qualification_answer(
    body: QualificationAnswerIn,
    request: Request,
    db: Annotated[Session, Depends(get_db)],
):
    _require_qualification_secret(request)
    phone = _normalize_phone(body.lead_phone)
    if not phone:
        raise HTTPException(status_code=422, detail="lead_phone inválido.")
    campaign = (
        db.query(Campaign)
    ...
    cfg = _ensure_config(db, body.tenant_id, body.campaign_id)
    if not cfg.enabled:
        raise HTTPException(status_code=400, detail="Qualificação está desativada nesta campanha.")

    step_key = body.step_key.strip().upper()
    steps = _ordered_steps(cfg)
    if step_key not in steps:
        raise HTTPException(status_code=422, detail=f"step_key inválido. Use: {', '.join(steps)}")
    ...
    score_delta, normalized = _score_answer(step_key, body.answer, cfg.scoring_rules_json or {})
    existing = (
        db.query(CampaignQualificationAnswer)
        .filter(
            CampaignQualificationAnswer.session_id == session.id,
    ...
    if next_step is None:
        ...
        session.classification = _classify(int(session.score_total or 0), cfg.classification_rules_json or {})
